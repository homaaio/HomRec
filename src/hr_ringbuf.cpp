...
/* `restrict` is C99 only; use compiler-specific spelling in C++ */
...
struct RingBuf {
    ...
    /* Both loads use acquire semantics for correct SPSC ordering */
...
HR_EXPORT size_t hr_rb_write(void *handle, const uint8_t * HR_RESTRICT data, size_t nbytes) {
    ...
    /* Compute contiguous space from the masked position, not the raw cursor */
    size_t mh    = rb->mask(h);
...
HR_EXPORT size_t hr_rb_read(void *handle, uint8_t * HR_RESTRICT dst, size_t nbytes) {
    ...
    /* Same wrap-around handling as in write() */
    size_t mt    = rb->mask(t);
...
/*
 * Clears the buffer contents as well as the cursors, so a consumer racing
 * with reset() can't read stale PCM data left over from a previous
 * recording session. seq_cst ensures both producer and consumer see the
 * reset immediately.
 */
