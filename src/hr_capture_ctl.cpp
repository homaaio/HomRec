/*
 * hr_capture_ctl.cpp - HomRec capture session controller
 *
 ...
 *
 * The UI layer calls this C API; the heavy capture/encode work is
 * delegated to hr_pipeline.cpp.
 */
...
/* -- Callbacks supplied by the caller (UI layer) --------------------------- */
...
/*
 * hr_ctl_update_stats
 * Called with the current output file size (bytes); emits the stats_cb
