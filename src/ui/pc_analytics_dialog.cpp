...
// WM_CREATE, values refreshed in place afterward rather than
// destroying/rebuilding the rows on every refresh.
    ...
    switch (msg) {
        ...
        case WM_CREATE: {
            ...
            for (const auto &row : kRows) {
                ...
                y += 26;
                // Small visual gap between the CPU/RAM/Disk groups.
                if (row.value_id == IDC_FIRST_LABEL + 1 || row.value_id == IDC_FIRST_LABEL + 4) y += 10;
            }
            HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"Refresh", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
