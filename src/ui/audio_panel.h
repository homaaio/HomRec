...
// wx widgets (LabeledSlider/ColorButton from themed_widgets.h, a wx-painted
...
public:
    ...
    // Fired when the panel's own close (\u2715) button is clicked. The panel
    // doesn't hide itself - main_frame owns whether it's shown (and keeps
    // the View menu checkbox in sync), same pattern as the overlays dock
...
private:
    ...
    wxStaticText *mic_label_ = nullptr, *sys_label_ = nullptr, *title_ = nullptr;
    LabeledSlider *mic_slider_ = nullptr, *sys_slider_ = nullptr;
    ColorButton *mic_mute_btn_ = nullptr, *sys_mute_btn_ = nullptr;
    ColorButton *close_btn_ = nullptr;
