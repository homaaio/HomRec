...
// Left-sidebar layout, stacking top-to-bottom (title -> START/PAUSE ->
// STATUS -> TIME -> STATS), all inset by kPad from the sidebar's
// left/right edges. Kept as constants shared between
...
void HomRecMainWindow::OnCreate() {
    ...
    // pad + 240 sidebar + 15 gap = 270) - see BuildLeftPanel()/
    // ComputeLayout() for the rest of this geometry.
    ...
    SetTimer(hwnd_, kStatsTimerId, 500, nullptr);          // stats/level polling, every 500ms
...
// The left sidebar is built out of a title, START/PAUSE buttons, a STATUS
// row (colored dot + text), a big TIME readout, and a STATS block (FPS +
// resolution) - each with its own bg/fg color pulled from the theme,
...
// into OnPaint by DrawLeftPanel(), using consistent colors, fonts, and
// stacked layout throughout. Fewer moving parts, and dynamic values
// (status text, dot color, timer, fps/res) just update a field +
// InvalidateRect instead of touching a widget directly.
...
// (editable in Advanced Settings as "Control+F9"-style strings) to the
// hotkey manager. Any string
...
void HomRecMainWindow::DoStop() {
    // Immediate "saving" feedback before the finalize work runs, even
    // though Stop() below runs synchronously rather than on a background
    // thread.
    ...
    pause_btn_bg_ = theme_.warning;
    EnableWindow(pause_btn_, FALSE);
    // Dot back to "error" (the same red used at idle/creation - see
    // BuildLeftPanel()'s comment) and status text back to "Ready".
    ...
    // KNOWN GAP: no "recording saved, open folder?" CustomMessageBox
    // popup wired here yet.
}
...
void HomRecMainWindow::DoPause() {
    ...
    rec_->TogglePause();
    if (state_.paused) {
        // pause_btn -> RESUME/success, dot -> warning, status -> "Paused".
...
void HomRecMainWindow::ToggleFullscreen() {
    // A real borderless fullscreen (no title bar/menu/border, covers the
    // whole monitor), not just a maximize. WS_OVERLAPPEDWINDOW is what
    // supplies the caption/border/thick-frame/menu chrome, so stripping it and
...
void HomRecMainWindow::DrawStartButton(DRAWITEMSTRUCT *dis) {
    ...
    // Button text uses the theme's "bg" color (dark text on the light
    // success/error button), not white.
