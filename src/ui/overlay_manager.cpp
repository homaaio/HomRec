...
std::wstring SummaryFor(const OverlayDef &ov) {
    ...
    std::string label = !ov.text.empty() ? ov.text
                       : !ov.image_path.empty() ? ov.image_path
                       : ov.input_json_path;
    oss << (ov.visible ? L"[on]  " : L"[off] ") << WideFromNarrow(ov.type) << L"  \""
        << WideFromNarrow(label) << L"\"  ("
        << ov.x << L"," << ov.y << L" " << ov.w << L"x" << ov.h << L")";
    return oss.str();
}

// "#RRGGBB" -> COLORREF, falls back to white on anything malformed.
COLORREF ColorRefFromHex(const std::string &hex) {
    auto nyb = [](char c) -> int {
    ...
    for (int i = 0; i < 6; ++i) {
        d[i] = nyb(hex[1 + i]);
        if (d[i] < 0) return RGB(255, 255, 255);
    }
    return RGB((d[0] << 4) | d[1], (d[2] << 4) | d[3], (d[4] << 4) | d[5]);
}

std::string HexFromColorRef(COLORREF c) {
    char buf[8];
...
enum {
    ...
    IDC_ED_COLOR, IDC_ED_OK, IDC_ED_CANCEL,
};
    ...
    switch (msg) {
        ...
        case WM_COMMAND: {
            ...
            if (id == IDC_ED_COLOR) {
                ...
                cc.hwndOwner = hwnd;
                cc.rgbResult = ctx->text_color;
                cc.lpCustColors = ctx->custom_colors;
                cc.Flags = CC_FULLOPEN | CC_RGBINIT;
                if (ChooseColorW(&cc)) {
                    ctx->text_color = cc.rgbResult;
                    SetWindowTextW(ctx->color_btn, WideFromNarrow(HexFromColorRef(ctx->text_color)).c_str());
                }
                return 0;
            }
            if (id == IDC_ED_OK) {
                ...
                ctx->ov->text_color = HexFromColorRef(ctx->text_color);
                int sel = (int)SendMessageW(ctx->type_combo, CB_GETCURSEL, 0, 0);
                ctx->ov->type = (sel == 1) ? "image" : (sel == 2) ? "webcam" : (sel == 3) ? "input_overlay" : "text";
                ctx->saved = true;
                DestroyWindow(hwnd);
...
bool EditOverlayDialog(HWND parent, HINSTANCE hInst, OverlayDef &ov) {
    ...
    int ex, ey, ew, eh;
    HrWin32Theme::CenteredWindowRect(340, 350, WS_POPUP | WS_CAPTION | WS_SYSMENU, ex, ey, ew, eh);
    ...
    SendMessageW(ctx.type_combo, CB_ADDSTRING, 0, (LPARAM)L"input_overlay (added via External Overlay)");
    SendMessageW(ctx.type_combo, CB_SETCURSEL,
                 ov.type == "image" ? 1 : ov.type == "webcam" ? 2 : ov.type == "input_overlay" ? 3 : 0, 0);

    y += 30; label(L"Text:", y);
    ctx.text_edit = edit(WideFromNarrow(ov.text), IDC_ED_TEXT, y);
    y += 30; label(L"Text Color:", y);
    ctx.text_color = ColorRefFromHex(ov.text_color);
    ctx.color_btn = CreateWindowExW(0, L"BUTTON", WideFromNarrow(HexFromColorRef(ctx.text_color)).c_str(),
                                     WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                                     100, y, 120, 22, hwnd, (HMENU)IDC_ED_COLOR, hInst, nullptr);
    HrWin32Theme::ThemeButton(ctx.color_btn);
...
bool ShowOverlayManager(HWND parent, HINSTANCE hInst, AppState &state) {
    ...
    HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"Add", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     12, 244, 70, 26, hwnd, (HMENU)IDC_ADD, hInst, nullptr));
    HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"Edit", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     90, 244, 70, 26, hwnd, (HMENU)IDC_EDIT, hInst, nullptr));
    HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"Delete", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     168, 244, 70, 26, hwnd, (HMENU)IDC_DELETE, hInst, nullptr));
    HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"Position...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     246, 244, 90, 26, hwnd, (HMENU)IDC_POSITION, hInst, nullptr));

    HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"Save", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
