...
// non-ASCII text), fall back to the system codepage rather than failing
// outright.
...
void LoadLogInto(HWND edit, const std::wstring &logPath) {
    ...
    // Win32 EDIT controls choke on bare \n, so CRLF-ize it first.
    std::wstring crlf;
...
LRESULT CALLBACK LogViewerProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    // dialog was opened (a brand new HWND/WM_CREATE each call, since it's
    // not reused) leaked one more GDI font object for the life of the
    // process. Tracked here the same way logPath already is, and cleaned
    // up alongside it in WM_DESTROY below.
    static HFONT logFont = nullptr;
    switch (msg) {
        case WM_CREATE: {
            ...
            HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"\U0001F504 Refresh", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                             12, 404, 110, 28, hwnd, (HMENU)IDC_LOG_REFRESH, hInst, nullptr));
            HrWin32Theme::ThemeButton(CreateWindowExW(0, L"BUTTON", L"\U0001F4C2 Open Folder", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        ...
        case WM_DESTROY:
            delete logPath;
            logPath = nullptr;
            if (logFont) { DeleteObject(logFont); logFont = nullptr; }
            return 0;
