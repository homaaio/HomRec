/*
 * hr_console.cpp - legacy standalone console window + Python-bridge design
 *
 * NOT CURRENTLY BUILT: this file is not part of the Makefile's SRCS list,
 * so it isn't compiled into hr.exe. The live developer console the app
 * actually uses today is ui/console_window.cpp (see ConsoleWindow,
 * instantiated from main_frame.cpp/main_window.cpp).
 *
 * This file predates that native rewrite: it implements the console as a
 * standalone DLL meant to be loaded by a Python host process, which calls
 * in via hr_con_set_command_cb() and gets dispatched commands forwarded
 * back out through forward_to_python()/hr_con_report_result(). None of
 * that bridge exists anymore, so the Python references below describe
 * this file's own original design, not a live dependency - left as-is
 * (rather than renamed/rewired) since the file isn't compiled and doing
 * so would just be churn on dead code.
 */
...
// split теперь корректно обрабатывает кавычки для #name="val with spaces"
...
bool dispatch(const std::wstring& raw) {
    ...
    // the "!" / "$" prefix used to be mandatory here - typing
    ...
    // !start --log handled inside cmd_start below
        ...
        // this gate used to only recognize --window/--rule/--ae/--all,
...
static void commit_input() {
    ...
    // предотвратить дублирование последней записи
...
static LRESULT CALLBACK wnd_proc(HWND hw, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WMA_EXEC: {
        // дренировать ВСЮ очередь за одно сообщение
        while (true) {
