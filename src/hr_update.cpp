/*
 * hr_update.cpp  -  HomRec update checker
 *
 * NOT CURRENTLY BUILT: this file is not part of the Makefile's SRCS list,
 * so it isn't compiled into hr.exe. Kept here for whenever an update
 * checker gets wired up; treat it as a reference/staging implementation,
 * not live code.
 *
 * check_for_updates() / _version_gt() : compares the latest GitHub release
 * tag against CURRENT_VERSION. Uses WinHTTP (Windows) or a libcurl stub
 * (non-Windows) to fetch the tag.
 *
 ...
 * Exposed for testing / external callers.
 */
