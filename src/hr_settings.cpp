/*
 * hr_settings.cpp - HomRec settings persistence engine
 *
 ...
 */
  ...
  #include <shlobj.h>
    ...
    // via -lshell32 on the command line instead (see the Makefile).
...
HR_EXPORT int  hr_settings_get_resolution_pct(const void *h) {
    return h ? static_cast<const HrSettings *>(h)->resolution_pct : 75;
}
