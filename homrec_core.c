...
/* ── BGRX → RGB conversion ─────────────────────────────────────────────────
 * mss returns BGRA (Windows) / BGRX — we strip the X/A and reorder channels.
 ...
 * src    : source  pixels (row-major, channels bytes per pixel)
 * dst    : dest    pixels (caller-allocated, dw*dh*channels bytes)
...
HR_EXPORT void hr_resize_bilinear(const uint8_t * restrict src,
                                   ...
                                   int ch)
{
    float x_ratio = (dw > 1) ? (float)(sw - 1) / (dw - 1) : 0.f;
    float y_ratio = (dh > 1) ? (float)(sh - 1) / (dh - 1) : 0.f;
    ...
    for (int y = 0; y < dh; ++y) {
        ...
        for (int x = 0; x < dw; ++x) {
            ...
            const uint8_t *p00 = src + (y0 * sw + x0) * ch;
            const uint8_t *p01 = src + (y0 * sw + x1) * ch;
            const uint8_t *p10 = src + (y1 * sw + x0) * ch;
            const uint8_t *p11 = src + (y1 * sw + x1) * ch;

            uint8_t *out = dst + (y * dw + x) * ch;
 ...
 * Much faster than audioop.rms() for large buffers because there is no
 * Python object overhead and the loop vectorises easily.
 */
                              ...
                              size_t n_pixels)
{
    ...
    for (size_t i = 0; i < n_pixels; ++i, b += 3, o += 4) {
        uint32_t a  = o[3];
        uint32_t a1 = 255 - a;
        b[0] = (uint8_t)((o[0] * a + b[0] * a1 + 127) >> 8);
        b[1] = (uint8_t)((o[1] * a + b[1] * a1 + 127) >> 8);
        b[2] = (uint8_t)((o[2] * a + b[2] * a1 + 127) >> 8);
    }
}
 ...
 * Writes "YYYY-MM-DD HH:MM:SS" into buf.  Returns number of chars written.
 */
                                  ...
                                  int ch)
{
    ...
    for (int y = 0; y < dh; ++y) {
        ...
        for (int x = 0; x < dw; ++x) {
            ...
            const uint8_t *s = src + (src_y * sw + src_x) * ch;
            uint8_t       *d = dst + (y    * dw + x     ) * ch;
            for (int c = 0; c < ch; ++c) d[c] = s[c];
        }
    }
}

/* ── Pixel-level RGB clamp-add (brightness boost preview) ───────────────────
 * Useful for very dark recordings — boost preview without touching capture.
 */
